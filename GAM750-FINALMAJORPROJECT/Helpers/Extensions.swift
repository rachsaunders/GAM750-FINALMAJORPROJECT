...
extension UIImage {
    ...
    var breadthRect: CGRect { return CGRect(origin: .zero, size: breathSize) }
    ...
    var circleMasked: UIImage? {
        ...
        //
        guard let cgImage = cgImage?.cropping(to: CGRect(origin: CGPoint(x: isLandscape ? floor((size.width - size.height) / 2) : 0, y: isPortrait ? floor((size.height - size.width) / 2) : 0), size: breathSize)) else { return nil }
